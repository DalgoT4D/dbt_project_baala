{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'baseline', 'women']
    )
}}

-- Staging model for Fbc Women Jhatingri Baseline Questionaire with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_fbc_women_jhatingri_baseline_questionaire_data as (
    select
        ...
        'fbc_women_jhatingri_baseline_questionaire' as source_table
        
    from {{ source('survey_raw_data', 'fbc_women_jhatingri_baseline_questionaire') }}
    where data is not null
)

select * from stg_fbc_women_jhatingri_baseline_questionaire_data
