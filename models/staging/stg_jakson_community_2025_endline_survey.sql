{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'endline', 'community']
    )
}}

-- Staging model for Jakson Community 2025 Endline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_jakson_community_2025_endline_survey_data as (
    select
        ...
        'jakson_community_2025_endline_survey' as source_table
        
    from {{ source('survey_raw_data', 'jakson_community_2025_endline_survey') }}
    where data is not null
)

select * from stg_jakson_community_2025_endline_survey_data
