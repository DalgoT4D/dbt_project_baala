{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'endline', 'school', 'boys']
    )
}}

-- Staging model for Fbc School Boys 2024 Endline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_fbc_school_boys_2024_endline_survey_data as (
    select
        ...
        'fbc_school_boys_2024_endline_survey' as source_table
        
    from {{ source('survey_raw_data', 'fbc_school_boys_2024_endline_survey') }}
    where data is not null
)

select * from stg_fbc_school_boys_2024_endline_survey_data
