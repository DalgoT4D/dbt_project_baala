{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'baseline', 'school']
    )
}}

-- Staging model for Caf Ajmer Students 2025 Baseline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_caf_ajmer_students_2025_baseline_survey_data as (
    select
        ...
        'caf_ajmer_students_2025_baseline_survey' as source_table
        
    from {{ source('survey_raw_data', 'caf_ajmer_students_2025_baseline_survey') }}
    where data is not null
)

select * from stg_caf_ajmer_students_2025_baseline_survey_data
