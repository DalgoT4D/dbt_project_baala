{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'baseline', 'teachers']
    )
}}

-- Staging model for Mosonie Dc Project Teachers 2025 Baseline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_mosonie_dc_project_teachers_2025_baseline_survey_data as (
    select
        ...
        'mosonie_dc_project_teachers_2025_baseline_survey' as source_table
        
    from {{ source('survey_raw_data', 'mosonie_dc_project_teachers_2025_baseline_survey') }}
    where data is not null
)

select * from stg_mosonie_dc_project_teachers_2025_baseline_survey_data
