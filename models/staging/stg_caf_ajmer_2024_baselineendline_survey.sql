{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'baseline', 'endline']
    )
}}

-- Staging model for Caf Ajmer 2024 Baselineendline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_caf_ajmer_2024_baselineendline_survey_data as (
    select
        ...
        'caf_ajmer_2024_baselineendline_survey' as source_table
        
    from {{ source('survey_raw_data', 'caf_ajmer_2024_baselineendline_survey') }}
    where data is not null
)

select * from stg_caf_ajmer_2024_baselineendline_survey_data
