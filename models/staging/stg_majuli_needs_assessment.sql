...
-- Staging model for Majuli Needs Assessment with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_majuli_needs_assessment_data as (
    select
        ...
        'majuli_needs_assessment' as source_table
        
    from {{ source('survey_raw_data', 'majuli_needs_assessment') }}
    where data is not null
)

select * from stg_majuli_needs_assessment_data
