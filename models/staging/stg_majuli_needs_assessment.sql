{
    config(
        materialized='view',
        tags=['staging', 'survey', 'needs_assessment']
    )
}

-- Staging model for Majuli needs assessment survey data
-- This model flattens the JSONB data and standardizes the structure

with majuli_needs_assessment_data as (
    select
        -- Standard fields
        _id,
        end,
        ...
        -- Needs_Assessment specific fields
        {{ extract_jsonb_value('data', 'community_needs') }} as community_needs,
        {{ extract_jsonb_value('data', 'intervention_recommendations') }} as intervention_recommendations,
        {{ extract_jsonb_value('data', 'resource_availability') }} as resource_availability,
        {{ extract_jsonb_value('data', 'priority_areas') }} as priority_areas,
    ...
    from {{ source('survey_raw_data', 'majuli_needs_assessment') }}
)

select * from majuli_needs_assessment_data
