{
    config(
        materialized='view',
        tags=['staging', 'survey', 'trainers']
    )
}

-- Staging model for Jakson CC trainers 2025 survey data
-- This model flattens the JSONB data and standardizes the structure

with jakson_cc_trainers_2025_survey_data as (
    select
        -- Standard fields
        _id,
        end,
        ...
        -- Trainers specific fields
        {{ extract_jsonb_value('data', 'qualification') }} as qualification,
        {{ extract_jsonb_value('data', 'training_experience') }} as training_experience,
        {{ extract_jsonb_value('data', 'specialization') }} as specialization,
        {{ extract_jsonb_value('data', 'trainer_name') }} as trainer_name,
    ...
    from {{ source('survey_raw_data', 'jakson_cc_trainers_2025_survey') }}
)

select * from jakson_cc_trainers_2025_survey_data
