{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'teachers']
    )
}}

-- Staging model for Jakson Cc Trainers 2025 Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_jakson_cc_trainers_2025_survey_data as (
    select
        ...
        'jakson_cc_trainers_2025_survey' as source_table
        
    from {{ source('survey_raw_data', 'jakson_cc_trainers_2025_survey') }}
    where data is not null
)

select * from stg_jakson_cc_trainers_2025_survey_data
