{{
    config(
        materialized='table',
        tags=['staging', 'survey', 'baseline', 'school', 'women']
    )
}}

-- Staging model for Fbc School Girls 2024 Baseline Survey with flattened JSONB data
-- This model flattens the JSONB data and standardizes the structure

with stg_fbc_school_girls_2024_baseline_survey_data as (
    select
        ...
        'fbc_school_girls_2024_baseline_survey' as source_table
        
    from {{ source('survey_raw_data', 'fbc_school_girls_2024_baseline_survey') }}
    where data is not null
)

select * from stg_fbc_school_girls_2024_baseline_survey_data
