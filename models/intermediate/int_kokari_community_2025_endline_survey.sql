{{
    config(
        materialized='table',
        tags=['intermediate', 'survey', 'endline', 'community']
    )
}}

-- Intermediate model for Kokari Community 2025 Endline Survey with business logic and transformations
...
{% set rel = ref('stg_kokari_community_2025_endline_survey') %}
...
{% set colname_menst_symp = get_column_by_substring_postgres(rel, 'What_symptoms_do_you_experienc_001') %}
...
') %}
{% set colname_menst_info = get_column_by_substring_postgres(rel, 'Where_do_you_usually_t_and_relationships') %}
