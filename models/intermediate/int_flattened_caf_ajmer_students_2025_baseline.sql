{{
    config(
        materialized='table',
        tags=['intermediate', 'flattened', 'survey', 'baseline', 'students']
    )
}}

-- Flattened intermediate model for caf_ajmer_students_2025_baseline_survey
-- This model flattens the JSONB data from the staging layer into individual columns
-- Focuses on student survey data from CAF Ajmer baseline
