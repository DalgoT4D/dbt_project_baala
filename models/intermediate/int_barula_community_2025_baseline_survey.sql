{{
    config(
        materialized='table',
        tags=['intermediate', 'survey', 'baseline']
    )
}}

-- Intermediate model for barula_community_2025_baseline_survey with business logic and transformations
...
{% set rel = ref('stg_barula_community_2025_baseline_survey') %}

{% set colname_per = get_column_by_substring_postgres(rel, '-') %}
{% set colname_age = get_column_by_substring_postgres(rel, '-') %}
